<template>
	<view class="refund" :style="{backgroundColor: orderList.length === 0 ? '#fff' : '' }">
		...
		>
			...
			>
				...
				>
					...
					<view class="right">
						...
						<text class="price base-color">￥ {{goodsItem.product_money}}</text>
					</view>
				</view>
          ...
          <button class="action-btn recom" @tap="navTo(`/pages/order/detail?id=${item.id}`)">订单详情</button>
				</view>
			</view>
      ...
      navTo (route) {
	      this.$mRouter.push({route});
		    ...
		    const params = {};
				...
				await this.$http.get(`${orderList}`, params).then(r => {
			    this.loading = false;
					if (type === 'refresh') {
