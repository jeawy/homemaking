	...
	<view class="rf-category">
		...
		<view class="nav">
			...
			<view class="top_search">
				<view class="top">
					<view class="top_left">
						<view class="left_one" @tap="tabClick('recommend')" :class="{ tab_active: currentIndex === 'recommend' }">推荐</view>
						<view class="left_two" @tap="tabClick('new')" :class="{ tab_active: currentIndex === 'new' }">最新</view>
						<view class="left_three" @tap="tabClick('nearby')" :class="{ tab_active: currentIndex === 'nearby' }">附近</view>
					</view>
					<view class="top_right">
						<view class="right_one" @tap="tabRightClick('screen')" :class="{ tab_active: tabIndex === 'screen' }">筛选</view>
						<image class="right_img" src="/static/houseKeeping/triangle.svg"></image>
						<view class="right_two" @tap="tabRightClick('classify')" :class="{ tab_active: tabIndex === 'classify' }">分类</view>
						<image class="right_img" src="/static/houseKeeping/triangle.svg"></image>
					</view>
				</view>
			</view>
				...
				<form @submit="confirm" @reset="clear">
					<view class="experience">
						...
						<view class="django_button">
							<view class="django_text" @tap="selectOne(index, item)" :class="{ django_active: selectone == index }" v-for="(item, index) in experienceLst"
					...
					<view class="language">
						...
						<view class="django_button">
							<view class="django_text" @tap="selectTwo(index, item,'search')" :class="{ django_active: item.checked == true }"
					...
					<view class="price">
						...
						<view class="django_button">
							<view class="django_text" @tap="selectThree(index, item)" :class="{ django_active: selectthree === index }"
					...
					<view class="tool">
						...
						<view class="django_button">
							<view class="django_text" @tap="selectFour(index, item)" :class="{ django_active: selectfour === index }" v-for="(item, index) in toolLst"
							 :key="index">{{ item.label }}
							</view>
						</view>
					</view>
	...
	export default {
      ...
      */
			...
			searchCate(data) {
				...
				this.$http.get(`${categoryList}`, params).then(({
						status,
						msg
					}) => {
						if (status === 0) {
